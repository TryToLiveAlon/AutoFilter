...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        try:
            ...
            asyncio.create_task(delete_after(lucypro, DELETE_TIME))
            return
...
#Deendayal403
        ...
        buttons = [[
            InlineKeyboardButton('× ᴀʟʟ ᴏᴜʀ ʟɪɴᴋꜱ ×', url="https://t.me/death_movie")
       ],[
            InlineKeyboardButton('• ɢʀᴏᴜᴘ •', url="https://t.me/+CI5BmjhnvXhjM2Fl"),
            InlineKeyboardButton('• ᴜᴘᴅᴀᴛᴇs •', url="t.me/death_movie")
       ],[
            InlineKeyboardButton('• sᴇʀɪᴇsғʟɪx •', url="https://t.me/+YNMtSIeOxVAxMGVl"),
            InlineKeyboardButton('• ᴍᴏᴠɪᴇғʟɪx •', url="https://t.me/+YNMtSIeOxVAxMGVl")
       ],[
            InlineKeyboardButton('• ᴀɴɪᴍᴇ ᴄʀᴜɪsᴇ •', url="https://t.me/+P18ePrep6rMyN2Nl")
       ],[ 
            InlineKeyboardButton('• ʙᴀᴄᴋ •', callback_data='start')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
    ...
    if imdb and imdb.get('poster'):
        try:
            ...
            if settings.get('auto_delete', True):
                asyncio.create_task(delete_after([hehe, message], DELETE_TIME))
            ...
            await m.delete()
            if settings.get('auto_delete', True):
                asyncio.create_task(delete_after([hmm, message], DELETE_TIME))
        except Exception as e:
            logger.exception(e)
            ...
            await m.delete()
            if settings.get('auto_delete', True):
                asyncio.create_task(delete_after([fek, message], DELETE_TIME))
    else:
        fuk = await message.reply_text(text=cap, reply_markup=InlineKeyboardMarkup(btn), disable_web_page_preview=True)
        await m.delete()
        if settings.get('auto_delete', True):
                asyncio.create_task(delete_after([fuk, message], DELETE_TIME))
                        ...
                        if btn == "[]":
                            ...
                            try:
                                if settings['auto_ffilter']:
                                    ...
                                    if settings.get('auto_delete', True):
                                            asyncio.create_task(delete_after(joelkb, DELETE_TIME))
                            except KeyError:
                                grpid = await active_connection(str(message.from_user.id))
                                await save_group_settings(grpid, 'auto_ffilter', True)
                                settings = await get_settings(message.chat.id)
